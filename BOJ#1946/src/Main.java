...
class Rank implements Comparable<Rank> {
	...
	public int compareTo(Rank rank) {
		//서류 결과 순위 기준으로 오름차순 정렬
		return this.first - rank.first;
	}
}
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < T; i++) {
			...
			//서류 결과 순위를 기준으로 오름차순으로 정렬
			Collections.sort(rank);
			
			int cnt = 1;
			int min = rank.get(0).second; //서류 결과 순위가 가장 높은 지원자의 면접 순위를 가져옴
			
			for (int j = 1; j < N; j++) {
				//특정 지원자의 면접 순위가 이전 지원자(서류 결과 순위가 높은 사람)의 면접 순위보다 높다면 신입사원 채용 -> cnt++
				if (min > rank.get(j).second) {
					min = rank.get(j).second;
					cnt++;
				}
			}
